"""Unit tests for voz_crawler.core.graph.embedding_sync.embed_batch.

The OpenAI client is mocked; embed_batch is a pure transform that returns
EmbedPatch objects (the caller writes them to the repository).
Tests verify:
...
def test_empty_input_returns_no_patches():
    client = MagicMock()
...
def test_single_batch_patches_have_correct_keys():
    ...
    patches: list[EmbedPatch] = embed_batch(client, items)
...
def test_custom_model_is_forwarded_to_api():
    items = _items(1)
    client = _openai_client([[0.1]])
    embed_batch(client, items, model="text-embedding-ada-002")
    _, kwargs = client.embeddings.create.call_args
...
def test_custom_model_is_stored_in_patch():
    items = _items(1)
    client = _openai_client([[0.1]])
    patches = embed_batch(client, items, model="my-custom-model")
...
def test_batching_calls_api_multiple_times():
    ...
    patches = embed_batch(client, items, batch_size=2)
...
def test_batching_correct_texts_per_batch():
    ...
    client.embeddings.create.side_effect = responses
    embed_batch(client, items, batch_size=2)
