from voz_crawler.core.entities.graph import EmbedItem, EmbedPatch
...
def embed_and_update(
    ...
    """Embed posts in batches and patch their embedding field in the graph store.

    ``client`` must be an OpenAI client (e.g. from openai.get_client(context)).
    ``repo`` must implement GraphRepository.
